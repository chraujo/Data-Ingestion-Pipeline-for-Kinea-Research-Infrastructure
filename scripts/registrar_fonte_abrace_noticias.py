...
# =============================================================================
# registrar_fonte_abrace_noticias.py
#
# Registro ÚNICO (roda uma vez). Fase 3 do fluxo de adição de fonte nova
# para "ABRACE Energia - Notícias". Já existia linha placeholder "ABRACE"
# em controle_fontes (catálogo original, nunca implementada -- source_id
# ="—"). UPDATE na linha existente em vez de INSERT, mesmo padrão de
# ONS/ANP/ABEGAS/ANATEL/Trata Brasil/CCEE.
#
# Campos preservados do placeholder original (não sobrescritos):
#   tipo_categoria = "Associações Setoriais (Energia)"
#   setor = "Energia"
...
#   (ABEGÁS também "Baixa"), sinal forte o suficiente pra não reclassificar.
# =============================================================================
...
linha_placeholder = spark.sql(f"""
    SELECT nome_fonte, source_id FROM {TABELA_FONTES} WHERE nome_fonte = 'ABRACE'
""").collect()

if len(linha_placeholder) != 1:
    raise RuntimeError(f"Esperava exatamente 1 linha 'ABRACE' em {TABELA_FONTES}, achei {len(linha_placeholder)}; abortando.")
...
print("[ok] 1 placeholder 'ABRACE' encontrado, nenhum source_id duplicado; prosseguindo.")
...
spark.sql(f"""
    ...
    SET
        source_id = '{SOURCE_ID}',
        status = 'Não iniciada',
        metodo_captura = 'Scraping HTML renderizado no servidor (WordPress) via dispatcher generico ingest-scraping, paginacao por path (/page/N)',
        notebooks_responsaveis = '{NOTEBOOK_NOME}',
        tasks_responsaveis = 'Ingest-scraping',
        notas = 'Data de publicacao nao aparece na listagem, so na pagina individual (time.entry-date, ISO 8601). Pagina individual tem dois <h1> (cabecalho generico do template "Noticias" + h1.entry-title com o titulo de verdade) -- extrair_titulo=None no CONFIGS_FONTES para nao herdar o bug do extrator generico de titulo (pegaria o cabecalho errado). Historico medio (40 paginas / ~480 posts) -- max_paginas=4 cobre so os ~48 mais recentes por execucao. Conteudo institucional (nota de imprensa, artigo de opiniao, cobertura de eventos/leiloes) -- sem filtro de relevancia na captura (proposital, fica para a etapa de NLP).'
    WHERE nome_fonte = 'ABRACE'
""")
print(f"[ok] linha 'ABRACE' atualizada com source_id='{SOURCE_ID}'")

# COMMAND ----------

# =============================================================================
# UPDATE em controle_notebooks -- acrescenta abrace_noticias em fontes_cobertas
# =============================================================================

...
mensagem_geral = MENSAGENS.get("geral", "")
aba_mensagens = f"""
    <div class="section-label">Mensagem da equipe</div>
    <div class="mensagem-geral">{escapar_multilinha(mensagem_geral) if mensagem_geral else '<span class="texto-fraco">Nenhuma mensagem cadastrada em MENSAGENS.md ainda.</span>'}</div>
"""

ABAS = [
    ("mensagens", "Mensagens", aba_mensagens),
    ("fontes", "Fontes", renderizar_aba_fontes(df_fontes, df_bloqueios)),
    ("notebooks", "Notebooks & Tasks", renderizar_aba_notebooks_tasks(df_notebooks, df_tasks)),
    ("bloqueios", "Bloqueios", renderizar_aba_bloqueios(df_bloqueios)),
    ("fases", "Fases & Entregáveis", renderizar_aba_fases()),
